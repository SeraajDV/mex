...
const Post = async ({ type }: { type?: "status" | "comment" }) => {
  ...
  const fileDetails = await getFileDetails("67b06dac432c47641670c66f");
  ...
  return (
    <div className="p-4 border-y-[1px] border-borderGray">
      {/* POST TYPE */}
      <div className="flex items-center gap-2 text-sm text-text_gray mb-2 from-bold">
        <svg
      ...
      <div className={`flex gap-4 ${type === "status" && "flex-col"}`}>
        ...
        <div className="flex-1 flex flex-col gap-2">
          ...
          <div className="w-full flex justify-between">
            <Link href={`/johnDoe`} className="flex gap-4">
              <div
                className={`${
                  type !== "status" && "hidden"
                } relative w-10 h-10 rounded-full overflow-hidden`}
              ...
              >
                ...
                <span
                  className={`text-text_gray ${type === "status" && "text-sm"}`}
                >
                  @johnDoe
                </span>
                {type !== "status" && (
                  <span className="text-text_gray">1 day ago</span>
                )}
              </div>
            </Link>
            <PostInfo />
          ...
          {type === "status" && (
            <span className="text-text_gray">8:41 PM · Dec 5, 2024</span>
          )}
          <PostInteractions />
        </div>
      </div>
